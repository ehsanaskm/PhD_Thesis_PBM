...
{  
  ...
  (
	...
	xi=max(abscissa1).value()/1000/(max(abscissa2).value()+1e-15)/1000;

        

        d_i=average(abscissa1).value()/1000;
        d_j=average(abscissa2).value()/1000;

	//u_i = 1.43*pow(average(epsilon).value()*d_i, 1./3);

	//u_j = 1.43*pow(average(epsilon).value()*d_j/1000, 1./3);
	u_i = 1.43*pow(min(average(epsilon).value(),epsMax_.value())*d_i, 1./3);

	u_j = 1.43*pow(min(average(epsilon).value(),epsMax_.value())*d_j, 1./3);
	...
	num = pow((0.75*(1.+pow(xi,2.))*(1.+pow(xi,3.))), 0.5);
        ...
        omega_coal = (M_PI/4.)*pow((d_i+d_j),2.)*u_i_j;
